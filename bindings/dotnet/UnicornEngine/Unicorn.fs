...
and Unicorn(arch: Int32, mode: Int32, binding: IBinding) =
    ...
    member this.MemWrite(address: Int64, value: Byte array) =
        match binding.MemWrite(_eng.[0], uint64 address, value, new UInt64(uint32 value.Length)) |> checkResult with
        | Some e -> raise e | None -> ()

    member this.MemRead(address: Int64, memValue: Byte array) =
        match binding.MemRead(_eng.[0], uint64 address, memValue, new UInt64(uint32 memValue.Length)) |> checkResult with
        | Some e -> raise e | None -> ()
