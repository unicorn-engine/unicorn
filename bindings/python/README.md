...
- Native support for Windows & *nix (with Mac OS X, Linux, *BSD & Solaris confirmed)
...
- `-e` enables editable mode, which gives you instant feedback instead of re-compiling every time.

Note that you should set up a valid build environment according to docs/COMPILE.md, but you do not necessarily need to build it because `setup.py` will do this for you.

## Python2 compatibility

By default, Unicorn python bindings work with Python3.7 and above, as it offers more powerful features which improve developing efficiency compared to Python2. However, Unicorn will only keep compatible with all features Unicorn1 offers regarding Python2 because it has reached end-of-life for more than 3 years at the time of writing this README. While offering all features for both Python2 & Python3 is desirable and doable, it inevitably costs too much effort to maintain and few users really rely on this. Therefore, we assume that if users still stick to Python2, previous Unicorn1 features should be enough. If you really want some new features Unicorn2 offers, please check and pull request to `unicorn/unicorn_py2`. We are happy to review and accept!
