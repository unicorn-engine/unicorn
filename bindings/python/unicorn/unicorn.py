...
class Uc(object):
    ...
    def reg_read(self, reg_id, opt=None):
        # type: (int, Union[None, int, ARMCPReg, ARM64CPReg]) -> Union[int, X86MMRReg, X86FPReg]
    ...
    def reg_write(self, reg_id, value):
        # type: (int, Union[int, ARMCPRegValue, ARM64CPRegValue, X86MMRReg, X86FPReg]) -> None
    ...
    def mmio_map(self, address, size,
                 read_cb, user_data_read,
                 write_cb, user_data_write):
        # type: (int, int, UC_MMIO_READ_TYPE, Any, UC_MMIO_WRITE_TYPE, Any) -> None
    ...
    @_catch_hook_exception
    def _hook_insn_cpuid_cb(self, handle, user_data):
        # type: (int, int) -> int
        # call user's callback with self object
        (cb, data) = self._callbacks[user_data]
        return cb(self, data)
    ...
    def hook_add(self, htype, callback, user_data=None, begin=1, end=0, arg1=0, arg2=0):
        # type: (int, UC_HOOK_CALLBACK_TYPE, Any, int, int, int, int) -> Any
        _h2 = uc_hook_h()
