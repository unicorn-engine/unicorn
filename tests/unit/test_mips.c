...
static void test_mips_el_ori(void)
{
    ...
    uc_common_setup(&uc, UC_ARCH_MIPS, UC_MODE_32 | UC_MODE_LITTLE_ENDIAN, code,
                    sizeof(code) - 1);
    OK(uc_reg_write(uc, UC_MIPS_REG_1, &r_r1, &reg_size));

    OK(uc_emu_start(uc, code_start, code_start + sizeof(code) - 1, 0, 0));

    OK(uc_reg_read(uc, UC_MIPS_REG_1, &r_r1, &reg_size));
...
static void test_mips_stop_at_branch(void)
{
    ...
    OK(uc_reg_read(uc, UC_MIPS_REG_PC, &r_pc, &reg_size));
...
static void test_mips_lwx_exception_issue_1314(void)
{
    ...
    OK(uc_reg_read(uc, UC_MIPS_REG_CP0_STATUS, &reg, &reg_size));
    reg |= (1 << 24);
    OK(uc_reg_write(uc, UC_MIPS_REG_CP0_STATUS, &reg, &reg_size));

    reg = 0;
    OK(uc_reg_write(uc, UC_MIPS_REG_1, &reg, &reg_size));
    OK(uc_reg_write(uc, UC_MIPS_REG_T9, &reg, &reg_size));
    reg = LEINT32(0xdeadbeef);
    OK(uc_mem_write(uc, 0x10000, &reg, 4));
    reg = 0x10000;
    OK(uc_reg_write(uc, UC_MIPS_REG_S3, &reg, &reg_size));

    OK(uc_emu_start(uc, code_start, code_start + sizeof(code) - 1, 0, 0));

    OK(uc_reg_read(uc, UC_MIPS_REG_T9, &reg, &reg_size));
