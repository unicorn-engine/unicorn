/**
 * Unicorn x86 self-modifying test
 *
                        ...
                        void *user_data)
{
    ...
    if (address == 0x60000029)
    {
        ...
        if (eax != 0x151494a0)
        {
            printf("FAIL: eax is not the expected 0x151494a0\n");
            print_registers(uc);
            //dump_stack_mem(uc);
            exit(-1);
        }
                       ...
                       void *user_data)
{
    ...
    if (type == 16) ctype = 'R';
    if (type == 17) ctype = 'W';
    printf("hook_mem32(%c): Address: 0x%lx, Size: %d, Value:0x%x\n", ctype, address, size, value);
...
int test_x86_32_imul_r_Ib(void)
{
    uc_engine *uc;
    uc_err err;
    ...
    if (err)
    {
        printf("Failed to uc_open() %s\n", err);
        exit(-1);
    }
  
    err = uc_mem_map(uc, PHY_STACK_REGION, CODE_SPACE, UC_PROT_ALL);
    if (err != UC_ERR_OK)
    {
        printf("Failed to uc_mem_map() %s\n", err);
        exit(-1);
    }
    ...
    if (err != UC_ERR_OK)
    {
        printf("Failed to uc_mem_write() %s\n", err);
        exit(-1);
    }
