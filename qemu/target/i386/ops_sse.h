...
void glue(helper_aesdec, SUFFIX)(CPUX86State *env, Reg *d, Reg *v, Reg *s)
{
    ...
    for (i = 0 ; i < 2 << SHIFT ; i++) {
        int j = i & 3;
        int k = (i & ~3) << 2;
        int b0 = AES_ishifts[4 * j + 0] + k;
        int b1 = AES_ishifts[4 * j + 1] + k;
        int b2 = AES_ishifts[4 * j + 2] + k;
        int b3 = AES_ishifts[4 * j + 3] + k;

        d->L(i) = rk.L(i) ^ bswap32(AES_Td0[st.B(b0)] ^
                                    AES_Td1[st.B(b1)] ^
                                    AES_Td2[st.B(b2)] ^
                                    AES_Td3[st.B(b3)]);
    }
...
void glue(helper_aesenc, SUFFIX)(CPUX86State *env, Reg *d, Reg *v, Reg *s)
{
    ...
    for (i = 0 ; i < 2 << SHIFT ; i++) {
        int j = i & 3;
        int k = (i & ~3) << 2;
        int b0 = AES_shifts[4 * j + 0] + k;
        int b1 = AES_shifts[4 * j + 1] + k;
        int b2 = AES_shifts[4 * j + 2] + k;
        int b3 = AES_shifts[4 * j + 3] + k;

        d->L(i) = rk.L(i) ^ bswap32(AES_Te0[st.B(b0)] ^
                                    AES_Te1[st.B(b1)] ^
                                    AES_Te2[st.B(b2)] ^
                                    AES_Te3[st.B(b3)]);
    }
