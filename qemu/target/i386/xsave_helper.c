...
void x86_cpu_xsave_all_areas(X86CPU *cpu, void *buf, uint32_t buflen)
{
    ...
    legacy = (X86LegacyXSaveArea *)(raw + e->offset);
    header = (X86XSaveHeader *)(raw + e->offset + sizeof(*legacy));
    ...
    if (e->size && e->offset) {
        ...
        bndreg = (XSaveBNDREG *)(raw + e->offset);
        bndcsr = (XSaveBNDCSR *)(raw + f->offset);
    ...
    if (e->size && e->offset) {
        ...
        opmask = (XSaveOpmask *)(raw + e->offset);
        zmm_hi256 = (XSaveZMM_Hi256 *)(raw + f->offset);
        ...
        hi16_zmm = (XSaveHi16_ZMM *)(raw + f->offset);
...
void x86_cpu_xrstor_all_areas(X86CPU *cpu, const void *buf, uint32_t buflen)
{
    ...
    legacy = (const X86LegacyXSaveArea *)(raw + e->offset);
    header = (const X86XSaveHeader *)(raw + e->offset + sizeof(*legacy));
    ...
    if (e->size && e->offset) {
        ...
        avx = (const XSaveAVX *)(raw + e->offset);
        for (i = 0; i < CPU_NB_REGS; i++) {
    ...
    if (e->size && e->offset) {
        ...
        bndreg = (const XSaveBNDREG *)(raw + e->offset);
        bndcsr = (const XSaveBNDCSR *)(raw + f->offset);
    ...
    if (e->size && e->offset) {
        ...
        opmask = (const XSaveOpmask *)(raw + e->offset);
        zmm_hi256 = (const XSaveZMM_Hi256 *)(raw + f->offset);
#ifdef TARGET_X86_64
        hi16_zmm = (const XSaveHi16_ZMM *)(raw + g->offset);
#endif
    ...
    if (e->size && e->offset) {
        ...
        pkru = (const XSavePKRU *)(raw + e->offset);
