...
static int s390_reg_read(struct uc_struct *uc, unsigned int *regs, void **vals, int count)
{
...
static int s390_reg_write(struct uc_struct *uc, unsigned int *regs, void *const *vals, int count)
{
    CPUS390XState* env = &(S390_CPU(uc->cpu)->env);
    int i;
...
DEFAULT_VISIBILITY
int s390_context_reg_read(struct uc_context *ctx, unsigned int *regs, void **vals, int count)
{
...
DEFAULT_VISIBILITY
int s390_context_reg_write(struct uc_context *ctx, unsigned int *regs, void *const *vals, int count)
{
    CPUS390XState* env = (CPUS390XState* )ctx->data;
    int i;
