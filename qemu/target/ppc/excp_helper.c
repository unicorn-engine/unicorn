                                        ...
                                        uint32_t lane)
{
    ...
    for (i = 0; i < 32; i++) {
        fxleft[i] = (rol16(xleft[i], 1) & rol16(xleft[i], 8)) ^
                    rol16(xleft[i], 2);
...
{
    uint64_t stage0_h = 0, stage0_l = 0;
    ...
    stage1_h = (uint64_t)helper_SIMON_LIKE_32_64(stage0_h >> 32,
                                                 key, 0) << 32;
    stage1_h |= helper_SIMON_LIKE_32_64(stage0_h, key, 1);
    stage1_l = (uint64_t)helper_SIMON_LIKE_32_64(stage0_l >> 32,
                                                 key, 2) << 32;
