...
static inline void gen_set_NZ64(TCGContext *tcg_ctx, TCGv_i64 result)
{
    ...
    tcg_gen_setcondi_i64(tcg_ctx, TCG_COND_NE, flag, result, 0);
    tcg_gen_trunc_i64_i32(tcg_ctx, tcg_ctx->cpu_ZF, flag);
    tcg_gen_shri_i64(tcg_ctx, flag, result, 32);
    tcg_gen_trunc_i64_i32(tcg_ctx, tcg_ctx->cpu_NF, flag);
...
static inline void gen_logic_CC(TCGContext *tcg_ctx, int sf, TCGv_i64 result)
{
    if (sf) {
        ...
        tcg_gen_trunc_i64_i32(tcg_ctx, tcg_ctx->cpu_ZF, result);
        tcg_gen_trunc_i64_i32(tcg_ctx, tcg_ctx->cpu_NF, result);
    }
...
static void gen_add_CC(DisasContext *s, int sf, TCGv_i64 dest, TCGv_i64 t0, TCGv_i64 t1)
{
    TCGContext *tcg_ctx = s->uc->tcg_ctx;
    if (sf) {
        ...
        tcg_gen_trunc_i64_i32(tcg_ctx, tcg_ctx->cpu_CF, flag);
        ...
        tcg_temp_free_i64(tcg_ctx, tmp);
        tcg_gen_shri_i64(tcg_ctx, flag, flag, 32);
        tcg_gen_trunc_i64_i32(tcg_ctx, tcg_ctx->cpu_VF, flag);
        ...
        tcg_gen_trunc_i64_i32(tcg_ctx, t0_32, t0);
        tcg_gen_trunc_i64_i32(tcg_ctx, t1_32, t1);
...
/* dest = T0 - T1; compute C, N, V and Z flags */
static void gen_sub_CC(DisasContext *s, int sf, TCGv_i64 dest, TCGv_i64 t0, TCGv_i64 t1)
{
    TCGContext *tcg_ctx = s->uc->tcg_ctx;
    if (sf) {
        /* 64 bit arithmetic */
...
static void gen_adc(DisasContext *s, int sf, TCGv_i64 dest, TCGv_i64 t0, TCGv_i64 t1)
{
    TCGContext *tcg_ctx = s->uc->tcg_ctx;
    TCGv_i64 flag = tcg_temp_new_i64(tcg_ctx);
...
static void gen_adc_CC(DisasContext *s, int sf, TCGv_i64 dest, TCGv_i64 t0, TCGv_i64 t1)
{
    TCGContext *tcg_ctx = s->uc->tcg_ctx;
    if (sf) {
        ...
        tcg_gen_trunc_i64_i32(tcg_ctx, tcg_ctx->cpu_CF, cf_64);
        gen_set_NZ64(tcg_ctx, result);

        tcg_gen_xor_i64(tcg_ctx, vf_64, result, t0);
        tcg_gen_xor_i64(tcg_ctx, tmp, t0, t1);
        tcg_gen_andc_i64(tcg_ctx, vf_64, vf_64, tmp);
        tcg_gen_shri_i64(tcg_ctx, vf_64, vf_64, 32);
        tcg_gen_trunc_i64_i32(tcg_ctx, tcg_ctx->cpu_VF, vf_64);
...
{
    g_assert(size <= 3);
    tcg_gen_qemu_st_i64(s->uc, source, tcg_addr, memidx, MO_TE + size);
}
                             ...
                             int size, bool is_signed, bool extend, int memidx)
{
    ...
    tcg_gen_qemu_ld_i64(s->uc, dest, tcg_addr, memidx, memop);
...
static void do_fp_st(DisasContext *s, int srcidx, TCGv_i64 tcg_addr, int size)
{
    ...
    if (size < 4) {
        tcg_gen_qemu_st_i64(s->uc, tmp, tcg_addr, get_mem_index(s), MO_TE + size);
    } else {
        TCGv_i64 tcg_hiaddr = tcg_temp_new_i64(tcg_ctx);
        tcg_gen_qemu_st_i64(s->uc, tmp, tcg_addr, get_mem_index(s), MO_TEQ);
        tcg_gen_ld_i64(tcg_ctx, tmp, tcg_ctx->cpu_env, fp_reg_hi_offset(s, srcidx));
        tcg_gen_addi_i64(tcg_ctx, tcg_hiaddr, tcg_addr, 8);
        tcg_gen_qemu_st_i64(s->uc, tmp, tcg_hiaddr, get_mem_index(s), MO_TEQ);
...
static void do_fp_ld(DisasContext *s, int destidx, TCGv_i64 tcg_addr, int size)
{
    ...
    if (size < 4) {
        ...
        tcg_gen_qemu_ld_i64(s->uc, tmplo, tcg_addr, get_mem_index(s), MO_TEQ);
        tcg_gen_addi_i64(tcg_ctx, tcg_hiaddr, tcg_addr, 8);
        tcg_gen_qemu_ld_i64(s->uc, tmphi, tcg_hiaddr, get_mem_index(s), MO_TEQ);
        tcg_temp_free_i64(tcg_ctx, tcg_hiaddr);
    }
                                ...
                                TCGv_i64 inaddr, int size, int is_pair)
{
    ...
    tmp = tcg_temp_new_i64(tcg_ctx);
    tcg_gen_qemu_ld_i64(s->uc, tmp, addr, get_mem_index(s), MO_TE + size);
    ...
    tcg_gen_qemu_st_i64(s->uc, cpu_reg(s, rt), addr, get_mem_index(s), MO_TE + size);
    if (is_pair) {
        TCGv_i64 addrhi = tcg_temp_new_i64(tcg_ctx);
...
static void disas_add_sub_imm(DisasContext *s, uint32_t insn)
{
    ...
    if (!setflags) {
        ...
        if (sub_op) {
            gen_sub_CC(s, is_64bit, tcg_result, tcg_rn, tcg_imm);
        } else {
            gen_add_CC(s, is_64bit, tcg_result, tcg_rn, tcg_imm);
        }
                      ...
                      enum a64_shift_type shift_type, TCGv_i64 shift_amount)
{
    ...
    case A64_SHIFT_TYPE_ROR:
        if (sf) {
            ...
            tcg_gen_trunc_i64_i32(tcg_ctx, t0, src);
            tcg_gen_trunc_i64_i32(tcg_ctx, t1, shift_amount);
...
static void disas_add_sub_ext_reg(DisasContext *s, uint32_t insn)
{
    ...
    if (!setflags) {
        ...
        if (sub_op) {
            gen_sub_CC(s, sf, tcg_result, tcg_rn, tcg_rm);
        } else {
            gen_add_CC(s, sf, tcg_result, tcg_rn, tcg_rm);
        }
    }
...
static void disas_adc_sbc(DisasContext *s, uint32_t insn)
{
    ...
    if (setflags) {
        gen_adc_CC(s, sf, tcg_rd, tcg_rn, tcg_y);
    } else {
        gen_adc(s, sf, tcg_rd, tcg_rn, tcg_y);
    }
}
...
static void disas_cc(DisasContext *s, uint32_t insn)
{
    ...
    tcg_tmp = tcg_temp_new_i64(tcg_ctx);
    if (op) {
        gen_sub_CC(s, sf, tcg_tmp, tcg_rn, tcg_y);
    } else {
        gen_add_CC(s, sf, tcg_tmp, tcg_rn, tcg_y);
    }
    tcg_temp_free_i64(tcg_ctx, tcg_tmp);
                       ...
                       unsigned int rn, unsigned int rd)
{
    ...
    if (sf) {
        ...
        tcg_gen_trunc_i64_i32(tcg_ctx, tcg_tmp32, tcg_rn);
                              ...
                              bool cmp_with_zero, bool signal_all_nans)
{
    ...
    if (is_double) {
        ...
        if (cmp_with_zero) {
            tcg_vm = tcg_const_i32(tcg_ctx, 0);
        } else {
...
static void handle_fp_1src_single(DisasContext *s, int opcode, int rd, int rn)
{
    ...
    case 0xe: /* FRINTX */
        gen_helper_rints_exact(tcg_ctx, tcg_res, tcg_op, fpst);
        break;
    case 0xf: /* FRINTI */
        gen_helper_rints(tcg_ctx, tcg_res, tcg_op, fpst);
        break;
...
static void handle_fp_1src_double(DisasContext *s, int opcode, int rd, int rn)
{
    ...
    case 0xe: /* FRINTX */
        gen_helper_rintd_exact(tcg_ctx, tcg_res, tcg_op, fpst);
        break;
    case 0xf: /* FRINTI */
        gen_helper_rintd(tcg_ctx, tcg_res, tcg_op, fpst);
        break;
                        ...
                        int opc, bool is_min, TCGv_ptr fpst)
{
    ...
    if (opc == 0xc) {
        if (is_min) {
            gen_helper_vfp_minnums(tcg_ctx, tcg_elt1, tcg_elt1, tcg_elt2, fpst);
        } else {
            gen_helper_vfp_maxnums(tcg_ctx, tcg_elt1, tcg_elt1, tcg_elt2, fpst);
        }
    } else {
        assert(opc == 0xf);
        if (is_min) {
            gen_helper_vfp_mins(tcg_ctx, tcg_elt1, tcg_elt1, tcg_elt2, fpst);
        } else {
            gen_helper_vfp_maxs(tcg_ctx, tcg_elt1, tcg_elt1, tcg_elt2, fpst);
        }
    }
}
...
static void disas_simd_across_lanes(DisasContext *s, uint32_t insn)
{
    ...
    if (!is_fp) {
        ...
        tcg_gen_trunc_i64_i32(tcg_ctx, tcg_elt1, tcg_elt);
        read_vec_element(s, tcg_elt, rn, 1, MO_32);
        tcg_gen_trunc_i64_i32(tcg_ctx, tcg_elt2, tcg_elt);
        ...
        tcg_gen_trunc_i64_i32(tcg_ctx, tcg_elt3, tcg_elt);

        do_minmaxop(s, tcg_elt2, tcg_elt3, opcode, is_min, fpst);

        do_minmaxop(s, tcg_elt1, tcg_elt2, opcode, is_min, fpst);

        tcg_gen_extu_i32_i64(tcg_ctx, tcg_res, tcg_elt1);
        tcg_temp_free_i32(tcg_ctx, tcg_elt1);
        tcg_temp_free_i32(tcg_ctx, tcg_elt2);
        tcg_temp_free_i32(tcg_ctx, tcg_elt3);
        tcg_temp_free_ptr(tcg_ctx, fpst);
    }
 ...
 */
static void handle_shri_with_rndacc(DisasContext *s, TCGv_i64 tcg_res, TCGv_i64 tcg_src,
...
static void handle_shli_with_ins(TCGContext *tcg_ctx, TCGv_i64 tcg_res, TCGv_i64 tcg_src,
                                 bool insert, int shift)
{
    if (insert) { /* SLI */
        tcg_gen_deposit_i64(tcg_ctx, tcg_res, tcg_res, tcg_src, shift, 64 - shift);
    } else { /* SHL */
        tcg_gen_shli_i64(tcg_ctx, tcg_res, tcg_src, shift);
    }
}

/* SRI: shift right with insert */
static void handle_shri_with_ins(TCGContext *tcg_ctx, TCGv_i64 tcg_res, TCGv_i64 tcg_src,
                                 int size, int shift)
{
    int esize = 8 << size;
    ...
    if (shift != esize) {
        tcg_gen_shri_i64(tcg_ctx, tcg_src, tcg_src, shift);
        tcg_gen_deposit_i64(tcg_ctx, tcg_res, tcg_res, tcg_src, 0, esize - shift);
    }
}
                                    ...
                                    int opcode, int rn, int rd)
{
    ...
    if (insert) {
        handle_shri_with_ins(tcg_ctx, tcg_rd, tcg_rn, size, shift);
    } else {
                                    ...
                                    int rn, int rd)
{
    ...
    handle_shli_with_ins(tcg_ctx, tcg_rd, tcg_rn, insert, shift);
                            ...
                            TCGv_i64 tcg_rd, TCGv_i64 tcg_rn, TCGv_i64 tcg_rm)
{
    ...
    case 0x11: /* CMTST, CMEQ */
        ...
        tcg_gen_and_i64(tcg_ctx, tcg_rd, tcg_rn, tcg_rm);
        tcg_gen_setcondi_i64(tcg_ctx, TCG_COND_NE, tcg_rd, tcg_rd, 0);
        tcg_gen_neg_i64(tcg_ctx, tcg_rd, tcg_rd);
        break;
    case 0x8: /* SSHL, USHL */
        if (u) {
            gen_helper_neon_shl_u64(tcg_ctx, tcg_rd, tcg_rn, tcg_rm);
        } else {
            gen_helper_neon_shl_s64(tcg_ctx, tcg_rd, tcg_rn, tcg_rm);
        }
        break;
    ...
    for (pass = 0; pass < passes; pass++) {
        ...
        case 0x16: /* FCVTN, FCVTN2 */
            ...
            if (size == 2) {
                ...
                tcg_gen_trunc_i64_i32(tcg_ctx, tcg_lo, tcg_op);
                gen_helper_vfp_fcvt_f32_to_f16(tcg_ctx, tcg_lo, tcg_lo, tcg_ctx->cpu_env);
                tcg_gen_shri_i64(tcg_ctx, tcg_op, tcg_op, 32);
                tcg_gen_trunc_i64_i32(tcg_ctx, tcg_hi, tcg_op);
                                ...
                                int opcode, int rd, int rn, int rm)
{
    ...
    if (size == 2) {
        ...
        for (pass = 0; pass < 2; pass++) {
            ...
            case 14: /* PMULL */
                assert(size == 0);
                gen_helper_neon_mull_p8(tcg_ctx, tcg_passres, tcg_op1, tcg_op2);
                break;
            default:
                g_assert_not_reached();
            }
...
static void do_narrow_high_u32(TCGContext *tcg_ctx, TCGv_i32 res, TCGv_i64 in)
{
    tcg_gen_shri_i64(tcg_ctx, in, in, 32);
    tcg_gen_trunc_i64_i32(tcg_ctx, res, in);
}

static void do_narrow_round_high_u32(TCGContext *tcg_ctx, TCGv_i32 res, TCGv_i64 in)
{
    tcg_gen_addi_i64(tcg_ctx, in, in, 1U << 31);
    do_narrow_high_u32(tcg_ctx, res, in);
}
...
static void disas_simd_3same_logic(DisasContext *s, uint32_t insn)
{
    ...
    for (pass = 0; pass < (is_q ? 2 : 1); pass++) {
        ...
        if (!is_u) {
            switch (size) {
            case 0: /* AND */
                tcg_gen_and_i64(tcg_ctx, tcg_res[pass], tcg_op1, tcg_op2);
                break;
            case 1: /* BIC */
                tcg_gen_andc_i64(tcg_ctx, tcg_res[pass], tcg_op1, tcg_op2);
                break;
            case 2: /* ORR */
                tcg_gen_or_i64(tcg_ctx, tcg_res[pass], tcg_op1, tcg_op2);
                break;
            case 3: /* ORN */
                tcg_gen_orc_i64(tcg_ctx, tcg_res[pass], tcg_op1, tcg_op2);
                break;
            }
        } else {
            if (size != 0) {
